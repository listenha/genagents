#!/bin/bash
# Run survey for multiple agents with multiple attempts
...
echo "=========================================="
echo "SURVEY MULTIPLE ATTEMPTS RUNNER"
echo "=========================================="
echo "Base path: ${BASE_PATH}"
echo "Agent range: ${AGENT_RANGE}"
echo "Number of attempts: ${NUM_ATTEMPTS}"
echo "Batch by section: ${BATCH_BY_SECTION}"
...
# Build base command
BASE_CMD="python3 -u Surveys/run_survey.py --base ${BASE_PATH}"
...
if [ "${BATCH_BY_SECTION}" = true ]; then
    BASE_CMD="${BASE_CMD} --batch-by-section"
fi
