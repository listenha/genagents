#!/usr/bin/env bash
# Patch Wavelength game attempts for the four agent populations to a target number
# of attempts per agent. Cleans dirty attempts (missing/null response for any
# header) then runs new attempts until each agent has TARGET_ATTEMPTS. Uses one
# header per inference (no batch). Run from repo root with venv active.
#
# Tunable: export TARGET_ATTEMPTS=3   (default 3)
# Usage: ./NCSA/run_wavelength_four_populations_patch.sh
...
run_patch_for_model() {
  local choice="$1"
  local base_path="$2"
  local log_file="${LOG_DIR}/wavelength_patch_${choice}_$(date +%Y%m%d_%H%M%S).log"
  ...
  python3 -u Wavelength_Game/run_wavelength_patch_attempts.py --base "$base_path" --target-attempts "$TARGET_ATTEMPTS" >> "$log_file" 2>&1
  echo "[$(date -Iseconds)] Done $choice exit_code=$?"
}

echo "========== Wavelength patch (four populations, target_attempts=$TARGET_ATTEMPTS, started $(date -Iseconds)) =========="
