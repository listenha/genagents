...
def analyze_header_distribution(agent_folders: List[Path], attempt_ids: Optional[List[int]], 
    ...
    print("Loading Wavelength game responses for distribution analysis...")
    all_metrics = []
...
def generate_distribution_summary_html(distribution_results: Optional[Dict]) -> Tuple[str, str]:
    """Generate summary HTML for distribution analysis."""
    summary_html = '<div class="summary-stats">'
    
    if distribution_results:
        n_agents = len(distribution_results['metrics']['agent_id'].unique())
        n_headers = len(distribution_results['header_stats'])
        n_edge_cases = len(distribution_results['edge_cases'])
        summary_html += f'''
            <div class="stat-card">
    ...
    edge_cases_html = ""
    if distribution_results and not distribution_results['edge_cases'].empty:
        edge_cases_html = "<h3>Edge Cases</h3>"
        edge_cases_html += distribution_results['edge_cases'].to_html(classes='data-table', index=False, escape=False)
    
    return summary_html, edge_cases_html


def generate_header_distribution_visualizations(distribution_results: Dict) -> str:
    """Generate all visualizations for headers (distribution analysis)."""
    ...
    df_metrics = distribution_results['metrics']
    header_stats = distribution_results['header_stats']
    header_cue_map = distribution_results.get('header_cue_map', {})
    ...
    # 4. Histograms for all headers with cue names in titles
    # Create subplots for headers (5 columns, calculate rows needed)
...
def analyze_consistency(base_path: str, agent_ids: List[str], attempt_ids: List[int]) -> Tuple[Dict, pd.DataFrame, pd.DataFrame, Optional[Dict]]:
    """
    Main analysis function - runs both consistency and distribution analysis.
    
    Returns:
        Tuple of (results, df_all, df_reasoning, distribution_results)
    """
    ...
    distribution_results = analyze_header_distribution(agent_folders, attempt_ids, Path(base_path))
    
    return results, df_all, df_reasoning_all, distribution_results
...
def generate_html_report(results: Dict, df_all: pd.DataFrame, output_path: str, 
    ...
    agent_level = results['agent_level']
    mean_stability_index = float(agent_level['stability_index'].mean() if not agent_level['stability_index'].isna().all() else 0)
    
    # Standard ICC (from header_aggregated)
    mean_icc_2k = float(header_agg['icc_2k'].mean() if not header_agg['icc_2k'].isna().all() else 0)
    mean_icc_3k = float(header_agg['icc_3k'].mean() if not header_agg['icc_3k'].isna().all() else 0)
    
    # Cronbach's Alpha
    mean_alpha = float(header_agg['cronbach_alpha'].mean() if not header_agg['cronbach_alpha'].isna().all() else 0)
    ...
    # Generate distribution content if available
    dist_summary_stats_html_escaped = None
    dist_summary_edge_cases_html_escaped = None
    dist_header_content_html_escaped = None
    dist_header_scripts_escaped = None
    if distribution_results is not None:
        dist_summary_html, dist_edge_cases_html = generate_distribution_summary_html(distribution_results)
        dist_summary_stats_html_escaped = json.dumps(dist_summary_html).replace('</script>', '<\\/script>')
        dist_summary_edge_cases_html_escaped = json.dumps(dist_edge_cases_html).replace('</script>', '<\\/script>')
        
        # Generate header distribution visualizations
        try:
            import plotly.graph_objects as go
            from plotly.subplots import make_subplots
            dist_header_html = generate_header_distribution_visualizations(distribution_results)
            
            # Extract divs and scripts separately (scripts inserted via innerHTML don't execute)
            def extract_divs_and_scripts(html_str):
                """Extract div elements and script content separately."""
                ...
                content_without_scripts = re.sub(r'<script[^>]*>.*?</script>', '', html_str, flags=re.DOTALL)
                
                return content_without_scripts, script_matches
            
            dist_content_html, dist_scripts = extract_divs_and_scripts(dist_header_html)
            
            # Escape content HTML for JavaScript
            dist_header_content_html_escaped = json.dumps(dist_content_html).replace('</script>', '<\\/script>')
            
            # Combine all scripts and escape for JavaScript
            dist_scripts_combined = '\n'.join(dist_scripts)
            dist_header_scripts_escaped = json.dumps(dist_scripts_combined).replace('</script>', '<\\/script>')
        except ImportError:
            dist_header_content_html_escaped = json.dumps("<p>Plotly not available for visualizations.</p>").replace('</script>', '<\\/script>')
            dist_header_scripts_escaped = None
    
    # Format values for display
    mean_stability_index_str = f"{mean_stability_index:.3f}"
    mean_icc_2k_str = f"{mean_icc_2k:.3f}"
    mean_icc_3k_str = f"{mean_icc_3k:.3f}"
    mean_alpha_str = f"{mean_alpha:.3f}"
    
    # Build conditional distribution tabs HTML
    distribution_tabs_html = ""
    if distribution_results is not None:
        distribution_tabs_html = """
            <button class="tab" onclick="showTab('distribution-summary', event)">Distribution Summary</button>
            <button class="tab" onclick="showTab('distribution-header-level', event)">Header Level Distribution</button>"""
    
    # Build conditional distribution content HTML
    distribution_content_html = ""
    if distribution_results is not None:
        distribution_content_html = """
        ...
        <div id="distribution-header-level" class="tab-content">
            <h2>Header Level Distribution</h2>
            <div id="distribution-header-content"></div>
        </div>
        """
    
    # Build conditional distribution JavaScript
    distribution_js_html = ""
    if distribution_results is not None and dist_summary_stats_html_escaped is not None:
        # Build the eval script part separately
        eval_script_part = ""
        if dist_header_scripts_escaped is not None:
            eval_script_part = f"""
            const distScripts = {dist_header_scripts_escaped};
            if (distScripts) {{
                try {{
                    eval(distScripts);
                }} catch (e) {{
                    console.error('Error executing distribution chart scripts:', e);
                }}
            }}
            """
        
        distribution_js_html = f"""
        // Distribution content
        if (document.getElementById('distribution-summary-stats')) {{
            document.getElementById('distribution-summary-stats').innerHTML = {dist_summary_stats_html_escaped};
        }}
        if (document.getElementById('distribution-summary-edge-cases')) {{
            document.getElementById('distribution-summary-edge-cases').innerHTML = {dist_summary_edge_cases_html_escaped};
        }}
        if (document.getElementById('distribution-header-content')) {{
            const headerContentDiv = document.getElementById('distribution-header-content');
            headerContentDiv.innerHTML = {dist_header_content_html_escaped};
            // Execute Plotly scripts manually (scripts inserted via innerHTML don't execute automatically)
            {eval_script_part}
        }}
        """
...
<body>
    <div class="container">
        ...
        <div class="tabs">
            ...
            <button class="tab" onclick="showTab('metric-diagnostics', event)">Metric Diagnostics</button>{distribution_tabs_html}
    </div>
        
        <div id="summary" class="tab-content active">
            <h2>Summary</h2>
            <div class="metric-box">
                <div class="metric-value">{mean_stability_index_str}</div>
                <div class="metric-label">Mean Stability Index</div>
            </div>
            <div class="metric-box">
                <div class="metric-value">{mean_icc_2k_str}</div>
                <div class="metric-label">Mean ICC(2,k)</div>
            </div>
            <div class="metric-box">
                <div class="metric-value">{mean_icc_3k_str}</div>
                <div class="metric-label">Mean ICC(3,k)</div>
            </div>
            <div class="metric-box">
                <div class="metric-value">{mean_alpha_str}</div>
    ...
    <script>
        ...
        """ + (distribution_js_html if distribution_js_html else "") + """
    </script>
</body>
</html>"""
    
    # Wrap Plotly scripts in DOMContentLoaded and extract JavaScript content (following bug report #5 and #8)
    wrapped_scripts = []
    if all_scripts:
    ...
    # Insert all scripts right after the Plotly CDN script tag (following bug report #5)
    if wrapped_scripts:
        all_scripts_html = '\n    '.join(wrapped_scripts)
    ...
    # Validate that f-string interpolation worked (following bug report pattern)
    if "{mean_stability_index_str}" in html_content:
        raise ValueError("ERROR: f-string interpolation failed - mean_stability_index_str not replaced in HTML template.")
...
def main():
    ...
    # Run analysis
    try:
        results, df_all, df_reasoning, distribution_results = analyze_consistency(
            args.base, agent_ids, attempt_ids
        )
        
        # Generate HTML report
        generate_html_report(results, df_all, args.output, agent_ids, attempt_ids, df_reasoning, distribution_results)
